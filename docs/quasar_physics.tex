...
and $\rho(\Delta\nu)$ the SpRS cross-section per unit length per unit bandwidth at frequency
offset $\Delta\nu = \nu_{\mathrm{cl}}-\nu_{\mathrm{q}}$ ($>0$ Stokes, $<0$ anti-Stokes).
The cross-section follows the spontaneous-Raman spectral shape of silica
\cite{dasilva2014,agrawal2019},
\begin{equation}
\boxed{\;\rho(\Delta\nu)=\rho_{\mathrm{peak}}\cdot g(|\Delta\nu|)\cdot A(\Delta\nu,T),\;}
\end{equation}
where $g(|\Delta\nu|)$ is the \emph{normalised} silica spontaneous-Raman spectral shape (peak
at ${\approx}13.2\,\mathrm{THz}$ / $440\,\mathrm{cm}^{-1}$, falling to zero near
$45\,\mathrm{THz}$; tabulated from Agrawal \cite{agrawal2019}, Fig.~8.1), and
$A(\Delta\nu,T)$ is the Bose--Einstein thermal asymmetry factor \cite{boyd2020}:
\begin{equation}
A(\Delta\nu,T)=\begin{cases}
n(|\Delta\nu|,T)+1, & \Delta\nu>0\;(\text{Stokes}),\\[2pt]
n(|\Delta\nu|,T), & \Delta\nu<0\;(\text{anti-Stokes}),
...
\end{equation}
with default $T=300\,\mathrm{K}$.  The Stokes/anti-Stokes asymmetry $\beta_{\mathrm{S}} >
\beta_{\mathrm{AS}}$ at equal $|\Delta\nu|$ follows automatically from Bose--Einstein
statistics rather than from two independent fitted slopes.  The absolute scale
$\rho_{\mathrm{peak}}$ is \emph{calibrated} so that the model reproduces
$\rho(35.4\,\mathrm{THz})\approx4\times10^{-11}\,\mathrm{(km\cdot nm)}^{-1}$ at the
$1310\to1550\,\mathrm{nm}$ Stokes offset \cite{eraerds2010,dasilva2014}; this gives
$\rho_{\mathrm{peak}}\approx1.19\times10^{-9}\,\mathrm{(km\cdot nm)}^{-1}$.

\textit{Note on the previous approximation.} An earlier form of eq.~(16) used a V-shaped
linear law $C_{\mathrm{R}}(\Delta\nu)\simeq C_0+\beta_{\mathrm{S/AS}}|\Delta\nu|$.  This is
a valid small-offset local approximation \cite{dasilva2014}, but the calibrated operating band
($12$--$43\,\mathrm{THz}$) sits entirely on the \emph{falling} side of the 13.2~THz silica
peak, where the linear-$\nu$ approximation is not accurate.  The physically-correct
spectral-shape form above supersedes it.  [LITERATURE-GROUNDED]
...
\begin{equation}
\boxed{\;p_{\mathrm{R}}=\frac{\eta\,T_{\mathrm{opt}}\,\Delta t}{h\nu}\,P_{\mathrm{R}},\qquad
Y_0 = 2\,p_{\mathrm{dark}}+p_{\mathrm{R}},\;}
\end{equation}
where $T_{\mathrm{opt}}\in(0,1]$ is the optical transmission of the detector filter and coupling
stage (in-line insertion loss between the fibre exit and the active detector element;
$T_{\mathrm{opt}}=1$ recovers the ideal lossless case) \cite{eraerds2010,dasilva2014},
...
\subsection{Calibration drift: gate overrotation}
\statusPhenom

Beyond coherence-time wear, each device node carries a gate-overrotation offset $\varepsilon(t)$
that grows linearly with time $t$ elapsed since the node's last calibration,
\begin{equation}
\varepsilon(t)=\varepsilon_0+\kappa_{\mathrm{drift}}\,t,
\end{equation}
where $\kappa_{\mathrm{drift}}$ [rad/s] is the calibration-drift rate (distinct from the
duty-cycle wear coefficient $\kappa$ above), and $\varepsilon_0$ is the residual overrotation
at the last calibration instant. This captures the slow, deterministic creep of device
parameters away from their calibration snapshot --- a process physically distinct from the
stochastic decoherence modelled by the $T_2(D)$ wear law. It is a calibrated engineering
heuristic rather than a derived result, and carries the same phenomenological status as the
$\kappa D$ wear term and the sensitivity matrix $\mathbf{S}$. Recalibration resets
$\varepsilon_0$ and the elapsed-time clock but does \emph{not} restore $T_2(D)$; wear and
drift accrue independently.
...
\bibitem{agrawal2019} G.~P. Agrawal,
\emph{Nonlinear Fiber Optics}, 6th ed. (Academic Press, 2019),
Fig.~8.1 — canonical silica Raman gain spectrum, used here for the normalised
spectral shape $g(|\Delta\nu|)$ with peak at ${\approx}13.2\,\mathrm{THz}$ ($440\,\mathrm{cm}^{-1}$).

\bibitem{boyd2020} R.~W. Boyd,
\emph{Nonlinear Optics}, 4th ed. (Academic Press, 2020), \S10.2 —
Stokes/anti-Stokes spontaneous-Raman intensity proportional to $(n+1)$ and $n$ respectively,
where $n$ is the Bose--Einstein phonon occupation number.
