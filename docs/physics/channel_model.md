...
**Exponential** — `K_α(τ) = (1/τ_α) · exp(−τ/τ_α)` for α ∈ {x, y, z}, returning a diagonal 3×3 matrix with per-axis time constants. The `1/τ_α` prefactor is the unit-area normalisation constant (∫₀^∞ K_α dτ = 1 per axis). Monotonic decay from `K(0) = diag(1/τx, 1/τy, 1/τz)`. Use this when the environment's influence fades with a single characteristic timescale per axis and you have no evidence of oscillatory backflow — the closest of the three to the Markovian limit while still carrying finite memory.

**Lorentzian** — `K(τ) = N_L · exp(−γ·τ) · cos(ω₀·τ) · I₃` where `N_L = (γ²+ω₀²)/γ` is the unit-area normalisation constant (derived from ∫₀^∞ exp(−γτ) cos(ω₀τ) dτ = γ/(γ²+ω₀²)). The cosine factor drives the kernel through zero and negative, producing sign-changing canonical rates and a positive non-Markovianity witness. Use this when modelling a resonant physical process — e.g. a mechanical resonance in the fibre mount — where the environment can transiently *return* coherence to the system.

**Gaussian** — `K(τ) = amplitude · N_G · exp(−τ²/(2σ²)) · I₃` where `N_G = √(2/(π·σ²))` is the unit-area normalisation constant (derived from ∫₀^∞ exp(−τ²/2σ²) dτ = σ√(π/2)). With `amplitude = 1.0`, each axis integrates to 1 over [0,∞). Always non-negative — no information backflow by itself. Use this to model a correlated bath fluctuation event with well-defined width `σ` and no oscillatory component.
